      ...
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <form method="POST" action="{{ route('tags.store') }}">
            @csrf
            <div class="mb-4">
              <label for="tag" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Tag</label>
              <input type="text" name="tag" id="tag" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 dark:bg-gray-700 leading-tight focus:outline-none focus:shadow-outline">
              @error('tags')
              <span class="text-red-500 text-xs italic">{{ $message }}</span>
              @enderror
